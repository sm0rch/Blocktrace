...
export async function emitWithArgs(viem, txHash, contract, eventName, expectedArgs) {
    ...
    const parsedLogs = await handleEmit(viem, Promise.resolve(txHashResult.value), contract, eventName);
    for (const { args: logArgs } of parsedLogs) {
        ...
        if (Array.isArray(logArgs)) {
            ...
            for (const [index, param] of expectedAbiEvent.inputs.entries()) {
                assert.ok(param.name !== undefined, `The event parameter at index ${index} does not have a name`);
