...
describe("Counter", async function () {
  ...
  // [~] Thêm retailerClient vào danh sách wallet
  const [adminClient, producerClient, inspectorClient, resolverClient, distributorClient, retailerClient] =
  ...
  const retailerAddress    = retailerClient.account.address;   // [+] mới
  ...
  async function deployAndSetupRoles() {
    ...
    const RETAILER_ROLE    = await counter.read.RETAILER_ROLE();   // [+] mới
    ...
    await counter.write.grantRole([RETAILER_ROLE,    retailerAddress],    { account: adminClient.account }); // [+] mới
