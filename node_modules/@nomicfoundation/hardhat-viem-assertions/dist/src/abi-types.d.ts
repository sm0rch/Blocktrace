import type { Abi, AbiItemArgs, AbiItemName, ContractErrorArgs, ContractErrorName, ContractEventName } from "viem";
/**
...
type WithPredicates<TArgs> = TArgs extends readonly unknown[] ? number extends TArgs["length"] ? unknown[] : {
 ...
 */
export type EventArgsOf<TAbi extends Abi, TEventName extends ContractEventName<TAbi>> = TEventName extends AbiItemName<TAbi> ? WithPredicates<AbiItemArgs<TAbi, TEventName>> : unknown[];
/** Args tuple for `TErrorName` on `TAbi`, with predicates allowed per position. */
export type ErrorArgsOf<TAbi extends Abi, TErrorName extends ContractErrorName<TAbi>> = WithPredicates<ContractErrorArgs<TAbi, TErrorName>>;
export {};
