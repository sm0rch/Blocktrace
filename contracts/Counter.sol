 ...
 *  - DISTRIBUTOR_ROLE   : cập nhật custody mốc vận chuyển, báo issue, neo evidence
 *  - RETAILER_ROLE      : xác nhận nhận hàng, báo issue, neo evidence, đề xuất return/replace
 *  - INSPECTOR_ROLE     : mở issue, khóa batch sang under_review, neo evidence
...
contract Counter is AccessControl {
    ...
    bytes32 public constant RETAILER_ROLE    = keccak256("RETAILER_ROLE");     // [+] mới
    bytes32 public constant INSPECTOR_ROLE   = keccak256("INSPECTOR_ROLE");
    bytes32 public constant RESOLVER_ROLE    = keccak256("RESOLVER_ROLE");
    bytes32 public constant AUDITOR_ROLE     = keccak256("AUDITOR_ROLE");      // [+] mới
    ...
    // ─────────────────────────────────────────────
    //  PLAN (báo lỗi): Mở issue
    // ─────────────────────────────────────────────

    /**
     * @notice Mở issue và khóa batch sang UnderReview.
     * @param batchId   ID batch có bất thường.
     * @param issueHash Hash của mô tả issue off-chain (loại lỗi, mức độ, mã reporter...).
     * @return issueId  ID của issue vừa tạo.
     *
     * @dev INSPECTOR_ROLE, RETAILER_ROLE, hoặc DISTRIBUTOR_ROLE đều được phép báo issue.
     *      Security Policy: "reportIssue → RETAILER_ROLE, DISTRIBUTOR_ROLE, hoặc INSPECTOR_ROLE".
     *      Batch chuyển Active → UnderReview ngay khi issue được mở.
    ...
    {
        // [~] Mở rộng từ onlyRole(INSPECTOR_ROLE) → 3 role được phép báo issue
        require(
     ...
     *
     * @dev INSPECTOR_ROLE, RETAILER_ROLE, hoặc DISTRIBUTOR_ROLE đều được neo evidence.
     *      Security Policy: các bên liên quan đều có thể đính kèm hash bằng chứng.
    ...
    function anchorEvidence(uint256 issueId, bytes32 evidenceHash)
        external
    {
        // [~] Mở rộng từ onlyRole(INSPECTOR_ROLE) → 3 role được neo evidence
        require(
            hasRole(INSPECTOR_ROLE,   msg.sender) ||
            hasRole(RETAILER_ROLE,    msg.sender) ||
            hasRole(DISTRIBUTOR_ROLE, msg.sender),
    ...
    //  (public → AUDITOR_ROLE và mọi bên đều đọc được)
    // ─────────────────────────────────────────────
